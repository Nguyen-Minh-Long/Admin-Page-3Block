...
function show(number) {
    ...
    var pagegraylist = JSON.parse(localStorage.getItem("graylist"));
    if (pagegraylist == null) {
        getGray();
    } else {
        ...
        appendData(pagegraylist.slice(start, end), pagegraylist.length);
        //Calculate the maximum number of pages
        pageButton(Math.ceil(pagegraylist.length / PAGE_SIZE));
        //Array containing all URL for Real Time Search
        var urlfilterrealtime = [];
        pagegraylist.forEach((t) => {
            //URL only
...
function appendData(data, length) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].isCheck == "none") {
            text += "</a></td><td><form><i class='fa fa-check-square-o'></i><input type='radio' id='yes' name='isCheck'" +
                " value='Yes' onclick=editYES('" +
                data[i].url + "')>" +
                "<label for='" + data[i]._id + "yes'>Yes</label><br>" +
                "<i class='fa fa-window-close-o'></i></i><input type='radio' id='no' name='isCheck'" +
                "value='No'onclick=editNo('" +
                data[i].url + "')>" +
                "<label for='" + data[i]._id + "no'>No</label></form></td>"
        ...
        text += "<td><button class='btn ripple'><a href='#' class='delete' data-toggle='modal'>" +
...
function findnumber(URLDEL, STT, ID) {
    //GET data from localstorage
    var pagegraylist = JSON.parse(localStorage.getItem("graylist"));
    for (var i = STT; i >= 0; i--) {
        if (pagegraylist[i] != null) {
            //Matching URL
            if (pagegraylist[i].url == URLDEL && pagegraylist[i]._id == ID) {
                //Remove the corresponding URL in object
                pagegraylist.splice(i, 1);
                //Upload to localstorage
                localStorage.setItem(
                    "graylist",
                    JSON.stringify(pagegraylist)
                );
                break;
            }
        }
...
function findnumberSearch(URLDEL, STT, ID) {
    //GET data from localstorage
    var pagegraylist = JSON.parse(localStorage.getItem("graylist"));
    for (var i = STT; i >= 0; i--) {
        if (pagegraylist[i] != null) {
...
//     if (validURL(newurl)) {
//         document.getElementById("notification").innerHTML = "<a href='#' class='center' style='color: #03a9f4'>"
...
//         // API Create New URL
//         fetch(
//             "https://api3blockserver.herokuapp.com/db/api/system/3block/createWhiteList",
//             {
//                 method: "POST",
...
function deleted(urldelete) {
    //API Soft Delete
    fetch(
        "https://api3blockserver.herokuapp.com/user/gray/system/3block/softDeleteOne",
        {
            method: "DELETE",
            headers: {
...
//Edit isCheck=yes
function editYES(URLEDIT) {
    fetch(
        "https://api3blockserver.herokuapp.com/user/gray/system/3block/confirmOne",
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
        ...
        .then(() => {
            // var pagegraylist = JSON.parse(localStorage.getItem("graylist"));
            // pagegraylist.forEach((t) => {
            //     if (t.url == URLEDIT && t._id == ID) {
            //         t.isCheck = "true";
            //     }
            // });
            // localStorage.setItem("graylist", JSON.stringify(pagegraylist));
            // show(state.page);
        })
        .catch(function (err) {
            console.log("error: " + err);
...
function TrashPage() {
    ...
    //API Trash GrayList
    fetch(
        "https://api3blockserver.herokuapp.com/user/gray/system/3block/getGrayTrash",
        {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
        ...
        .then(function (data) {
            ...
            //Render
            window.location.pathname = "./GrayList/Trash";
        })
        .catch(function (err) {
            console.log("error: " + err);
        });
