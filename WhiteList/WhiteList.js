...
function show(number) {
    ...
    var pagewhitelist = JSON.parse(localStorage.getItem("whitelist"));
    if (pagewhitelist == null) {
        ...
        appendData(pagewhitelist.slice(start, end), pagewhitelist.length);
        //Calculate the maximum number of pages
        pageButton(Math.ceil(pagewhitelist.length / PAGE_SIZE));
        //Array containing all URL for Real Time Search
        var urlfilterrealtime = [];
        pagewhitelist.forEach((t) => {
            //URL only
...
function appendData(data, length) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        text += "<td>" + timeUpdated.toLocaleString() +
            "</td>" +
...
function findnumber(urldelete, stt) {
    ...
    for (var i = stt; i >= 0; i--) {
        if (pagewhitelist[i] != null) {
            //Matching URL
            if (pagewhitelist[i].url == urldelete) {
                //Remove the corresponding URL in object
                pagewhitelist.splice(i, 1);
                //Upload to localstorage
                localStorage.setItem(
                    "whitelist",
                    JSON.stringify(pagewhitelist)
                );
                break;
            }
        }
...
function addURL() {
    ...
    if (validURL(newurl)) {
        ...
        // API Create New URL
        fetch(
            "https://admin3blockapi.herokuapp.com/db/api/system/3block/createWhiteList",
            {
                method: "POST",
                headers: {
...
function deleted(urldelete) {
    //API Soft Delete
    fetch(
        "https://api3blockserver.herokuapp.com/db/api/system/3block/softDeleteWhite",
        {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
...
function TrashPage() {
    ...
    fetch(
        ...
        .then(function (data) {
            ...
            //Render
            window.location.pathname = "./WhiteList/Trash";
        })
        .catch(function (err) {
            console.log("error: " + err);
        });
