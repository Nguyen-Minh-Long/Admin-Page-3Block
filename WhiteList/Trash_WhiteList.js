...
//Show Trash WhiteList URL
function show(number) {
    state.page = number; //Current Page
    //GET data from localstorage
    var WhiteListTrash = JSON.parse(localStorage.getItem("trash"));
    if (WhiteListTrash == null) {
        refresh();
        show(state.page);
    } else {
        ...
        if (WhiteListTrash.length < 1) {
            ...
            appendData(WhiteListTrash.slice(start, end), WhiteListTrash.length);
            //Calculate the maximum number of pages
            pageButton(Math.ceil(WhiteListTrash.length / PAGE_SIZE));
            //Bold current page
            document
...
function appendData(data, length) {
    ...
    let text = "<table class='container'>"
        + "<thead><tr><th><h1>No.</h1></th><th><h1>URL</h1></th>"
        + "<th><h1>DeletedAt</h1></th><th><h1>Action</h1></th></tr></thead>"
        + "<tbody><tr>"
    for (var i = 0; i < data.length; i++) {
        text +=
            ...
            data[i].url +
            "</a></td>"
        var timeUpdated = new Date(data[i].updatedAt)
        text += "<td>" + timeUpdated.toLocaleString() + "</td>"
...
function findnumber(urledit, stt, index) {
    //GET data from localstorage
    var pageWhiteList = JSON.parse(localStorage.getItem("trash"));
    for (var i = stt; i >= 0; i--) {
        if (pageWhiteList[i] != null) {
            //Matching URL
            if (pageWhiteList[i].url == urledit) {
                //GET data from localstorage
                var WhiteListTrash = JSON.parse(localStorage.getItem("trash"));
                //Remove the corresponding URL in object
                WhiteListTrash.splice(i, 1);
                //Upload to localstorage
                localStorage.setItem("trash", JSON.stringify(WhiteListTrash));
                break;
            }
        }
    }
...
//Button for Trash WhiteList (Search)
...
function destroy(urldestroy) {
    //API Real Delete
    fetch(
        "https://api3blockserver.herokuapp.com/db/api/system/3block/realDeleteWhite",
        {
            method: "DELETE",
            headers: {
...
function restore(urlrestore) {
    //API Restore
    fetch(
        "https://api3blockserver.herokuapp.com/db/api/system/3block/restoreWhite",
        {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
...
function refresh() {
    ...
    //API Trash WhiteList
    fetch(
        "https://api3blockserver.herokuapp.com/db/api/system/3block/getWhiteTrash",
        {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
